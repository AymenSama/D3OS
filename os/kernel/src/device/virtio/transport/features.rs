/* ╔═════════════════════════════════════════════════════════════════════════╗
   ║ Module: features.rs                                                     ║
   ╟─────────────────────────────────────────────────────────────────────────╢
   ║ Defines status and feature flags for VirtIO device configuration.       ║
   ║                                                                         ║
   ║ - DeviceStatusFlags: status byte written by the driver to the device    ║
   ║   during initialization (reset, acknowledge, failed, etc.).             ║
   ║ - VirtioFeatures: negotiated feature bits per VirtIO 1.3 specification. ║
   ║   Covers capabilities such as INDIRECT_DESC, VERSION_1, EVENT_IDX, etc. ║
   ║                                                                         ║
   ║ Reference:                                                              ║
...
bitflags::bitflags! {
    ...
    pub struct DeviceStatusFlags: u8 {
        /// Indicates that the device is not ready to be used. The driver should not attempt to use the device until this bit is cleared.
        const RESET = 0;
